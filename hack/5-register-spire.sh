...
echo "Registering SPIRE Workload for API Gateway (Step 4.5)"
echo "====================================================="
...
echo "Checking for existing API Gateway registration..."
# Check if entry exists to avoid duplicates. Identify by SPIFFE ID.
EXISTING_ENTRY=$(kubectl exec -n spire spire-server-0 -- /opt/spire/bin/spire-server entry show -spiffeID spiffe://securepay.dev/api-gateway 2>/dev/null | grep "Entry ID" | awk '{print $NF}' | head -n 1 | tr -d '\r' || true)

if [ ! -z "$EXISTING_ENTRY" ]; then
    echo "Entry for api-gateway already exists (ID: $EXISTING_ENTRY). Deleting to recreate..."
    kubectl exec -n spire spire-server-0 -- /opt/spire/bin/spire-server entry delete -entryID $EXISTING_ENTRY
fi

echo "Creating registration entry for API Gateway..."
# Registering the workload
# SPIFFE ID: spiffe://securepay.dev/api-gateway
# Parent ID: The attested Agent ID
# Selectors: Kubernetes Namespace (default) and ServiceAccount (api-gateway)
# The deployment for api-gateway is expected to happen in the 'default' namespace with the 'api-gateway' ServiceAccount.
kubectl exec -n spire spire-server-0 -- \
    /opt/spire/bin/spire-server entry create \
    -spiffeID spiffe://securepay.dev/api-gateway \
    -parentID $AGENT_ID \
    -selector k8s:ns:default \
    -selector k8s:sa:api-gateway

echo "API Gateway workload registered successfully."
