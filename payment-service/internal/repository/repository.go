...
func (r *PostgresRepository) UpdatePaymentStatus(ctx context.Context, paymentID string, status models.PaymentStatus) error {
	query := `
	...
	result, err := r.db.ExecContext(ctx, query, string(status), paymentID)
	if err != nil {
